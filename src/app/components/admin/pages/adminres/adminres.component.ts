...
export class AdminresComponent implements OnInit {
  ...
  createActualite(){
  this.ressourcesService.createActualite(this.actualiteRes).subscribe(data=>{
      console.log(data)
  ...
  getActualitesadmin(){
    this.actualiteRes = new ActualiteRes();
    this.ressourcesService.getoffresAll().subscribe(data => {
      if(data != null){
 ...
 var id= Object.values(act)[0]
  ...
  editActualite(act: ActualiteRes) {
    var id= Object.values(act)[0]
    this.ressourcesService.findActualiteById(id).subscribe(data=>{
